...
# raster layer that has the same structure as the raster layer output
# by the covariate module
PredictNewRasterMap <- function(.model, .ras, raster = .ras){
```

It is important to have default values for all user defined parameters so that your module can be tested when you upload it to the zoon website. Here we set our defualt 'new area' raster to be the same as the raster used to create the model. Clearly this is not how we envisage a user useing the module (unless they geniuinely wanted to predict back to the same area), however this ensures that this module will always work with its default, no matter what workflow it is placed in. 
...
BuildModule(PredictNewRasterMap,
            ...
            author = 'Z.O. Onn',
            email = 'zoon@zoon-zoon.com',
            paras = list(raster = paste('A raster brick, layer or stack in',
                                        'the same format as the raster provided',
                                        'by the covariate module. Predicted values',
                                        'will be estimated for this raster useing',
...
The input arguments and return values of modules are strict. However, any module type can have additional named input arguments, provided they have default values. A lot of the data frames include '+ covariates'. This indicates that the number of covariate columns is flexible.
<a id="occIO"></a>
...
*Out*: `RasterLayer` or `RasterStack` object
<a id="proIO"></a>
...
* `ras`: A `RasterLayer` or `RasterStack` object of covariate rasters
...
### Output
In: A list named **`.model`** with 2 named elements:
