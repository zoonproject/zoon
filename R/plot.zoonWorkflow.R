  ...
  for (i in 1:5) {
    ...
    for (N in 1:NoOfModules[i]) {
      ...
      if (IsList[i]) {
        ...
        segments(x1[i] - (xG * 0.33),
                 yStart + 0.5 * yH, 
                 x1[i] - xG * 0.33,
                 yStart + (0.5 * yH) + ((NoOfModules[i] - 1) * (yH + yG)),
                 lwd = lW, col = clrs[ColCode])
        
        segments(x2[i] + (xG * 0.33),
                 yStart + 0.5 * yH, 
                 x2[i] + xG * 0.33,
                 yStart + (0.5 * yH) + ((NoOfModules[i] - 1) * (yH + yG)), 
                 lwd = lW, col = clrs[ColCode])
        ...
        if (IsChain[i]) {
          ...
          if (N != NoOfModules[i]) {
            segments(0.5 * (x1[i] + x2[i]), yNow + yH,
      ...
      if (ColCode %% 2 == 0) {
        ...
        text(x1[1] + (0.5 * (x2[1] - x1[1])), - 18,
             "** Modules not found in the repoistory",
             col = "cornsilk4", adj = 0, cex = 0.8)
        
        rect(x1[1] + (0.5 * (x2[1] - x1[1])), wInc,
             x2[1] + (0.5 * (x2[1] - x1[1])), wInc + yH,
             col = clrs[ColCode], border = clrs[ColCode],
             lwd = lW)
        
        text(x1[1] + (0.5 * (x2[1] - x1[1])), wInc + 0.5 * yH,
             ModuleNames[[ModuleInc]],
             col = clrsTXT, adj = 0,
             cex = 0.75)
...
CallLister <- function (callList) {
  ...
  repoModuleList <- unlist(GetModuleList(renew = TRUE),
                           use.names = FALSE)
