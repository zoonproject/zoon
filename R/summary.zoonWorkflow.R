...
summaryHTML <- function(workflow){
  ...
  callHeader <- paste('<h1>Zoon Workflow summary</h1><br>',
                      '<p>Call:', workflow$call, '<p/>')
  
  occurrenceHeader <- '<hr><h2>Occurrence modules</h2>'
  
  occurrenceCall <- paste('<p>',
                          'Parameterisation: ',
                          substr(workflow$call,
...
occHTML <- function(occ){
  ...
  HTML <- paste('<h3>Module:', attr(occ, 'call_path'), '</h3>',
...
covHTML <- function(cov){
  ...
  HTML <- paste('<h3 style="clear: both;">Module:', attr(cov, 'call_path'), '</h3>',
                '<div style="clear: both; float: left; width: 200px;">',
...
htmlTable <- function(dataFrame, maxWidth = 600, maxHeight = 300){
  ...
  tableHTML <- paste('<div style="float: left; max-width: ', maxWidth, 'px; max-height: ', maxHeight, 'px; overflow: scroll;>">',
...
proHTML <- function(pro){
  ...
  HTML <- paste('<h3 style="clear: both;">Module:', attr(pro, 'call_path')$process, '</h3>',
...
modHTML <- function(mod){
  ...
  HTML <- paste('<h3 style="clear: both;">Module:', attr(mod, 'call_path')$model, '</h3>',
...
outHTML <- function(out){
  ...
  if('plot' %in% suppressWarnings({attr(methods(class = class(out)), 'info')$generic})){
  ...
  } else if('summary' %in% suppressWarnings({attr(methods(class = class(out)), 'info')$generic})){
