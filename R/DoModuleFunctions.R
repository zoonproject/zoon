...
DoOutputModules <- function(output.module, outputName, process.module,
  ...
  if (!identical(attr(output.module, "chain"), TRUE)) {
    ...
    if (length(process.output) == length(model.output)) {
      ...
      output.output <- unlist(future_lapply(
        model.output,
        function(y) future_lapply(
          outputName,
          function(x) {
...
DoModelModules <- function(model.module, modelName, process.output, e) {
  ...
  if (length(model.module) > 1) {
    model.output <- future_lapply(modelName,
                                  FUN = DoModelList,
                                  e = e,
                                  process.output = process.output)
  } else {
    model.output <- future_lapply(process.output,
                                  FUN = DoModelNotList,
                                  e = e,
                                  modelName = modelName)
  }
...
DoProcessModules <- function(process.module, processName, data, e) {
  ...
  if (!identical(attr(process.module, "chain"), TRUE)) {
    if (length(processName) > 1) {
      process.output <- future_lapply(processName, FUN = DoProcessList, e = e)
    } else {
      process.output <- future_lapply(data, FUN = DoProcessNotList, e = e)
    }
  } else {
    ...
    for (p in seq_along(processName)) {
      process.output <- future_lapply(process.output,
                               FUN = DoProcessChain,
                               p = p,
                               e = e)
    }
    ...
    call_path_process_chain <- function(x) {
      attr(x, "call_path") <- c(
        ...
        process = paste(
          "Chain(",
          paste(
            future_lapply(processName, function(x) x$module),
            collapse = ", "
          ),
          ")", sep = ""
        )
    ...
    process.output <- future_lapply(process.output, call_path_process_chain)
